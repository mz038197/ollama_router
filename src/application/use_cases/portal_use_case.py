...
class PortalUseCase:
    ...
    def create_session(self, teacher_id: int, class_id: int) -> dict[str, Any]:
        self._assert_teacher(teacher_id)
        ...
        return self.repo.create_class_session(class_id, teacher_id)
    ...
    def admin_update_user(self, admin_id: int, user_id: int, role: str | None, status: str | None) -> dict[str, Any] | None:
        self._assert_admin(admin_id)
        return self.repo.update_user(user_id, role=role, status=status)
