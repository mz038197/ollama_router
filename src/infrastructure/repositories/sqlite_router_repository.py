...
class SqliteRouterRepository:
    ...
    def _role_for_email(self, email: str) -> tuple[str, bool]:
    ...
    def upsert_google_user(self, email: str, name: str, google_sub: str | None = None) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            ...
            return dict(conn.execute("SELECT * FROM users WHERE email = ?", (email.lower(),)).fetchone())
    ...
    def update_user(self, user_id: int, role: str | None = None, status: str | None = None) -> dict[str, Any] | None:
        fields = []
        values: list[Any] = []
        if role:
            fields.append("role = ?")
    ...
    def issue_long_lived_key(self, user_id: int) -> str:
        raw = "or_sk_" + secrets.token_hex(32)
        now = _dt(_utc_now())
        with self._connect() as conn:
    ...
    def create_class_session(self, class_id: int, created_by: int) -> dict[str, Any]:
        klass = self.get_class(class_id)
        if not klass or klass["status"] != "active":
            raise ValueError("class is not active")
        expires_at = _utc_now() + timedelta(hours=int(klass["api_key_ttl_hours"]))
    ...
    def log_prompt(self, auth: AuthContext | None, raw_prompt: str, final_prompt: str, model: str, status: str, client_ip: str | None) -> None:
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO prompt_logs(user_id, class_id, session_id, raw_prompt, final_prompt, model, status, client_ip, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
